  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4">
          <div className="text-xs text-muted-foreground">Total harvest (season)</div>
          <div className="text-2xl font-bold mt-1">{totalKg.toFixed(1)}<span className="text-sm font-normal text-muted-foreground"> kg</span></div>
        </Card>
        <Card className="p-4">
          <div className="text-xs text-muted-foreground flex items-center gap-1"><DollarSign className="size-3" /> Revenue from orders</div>
          <div className="text-2xl font-bold mt-1 text-primary">{revenue.toLocaleString(undefined,{maximumFractionDigits:0})} <span className="text-sm font-normal text-muted-foreground">ETB</span></div>
          <div className="text-[10px] text-muted-foreground mt-0.5">{collected.toLocaleString()} ETB collected · {customerOrders.length} orders</div>
        </Card>
        <Card className="p-4">
          <div className="text-xs text-muted-foreground">Plant mortality</div>
          <div className="text-2xl font-bold mt-1">2.4<span className="text-sm font-normal text-muted-foreground"> %</span></div>
          <div className="text-[10px] text-primary">Below 5% target</div>
        </Card>
        <Card className="p-4">
          <div className="text-xs text-muted-foreground">Disease incidents</div>
          <div className="text-2xl font-bold mt-1">{diseases.length}</div>
          <div className="text-[10px] text-muted-foreground">{diseases.filter(d=>d.status==="resolved").length} resolved</div>
        </Card>
      </div>
      ...
      <Tabs defaultValue="daily">
        ...
        <TabsContent value="weekly" className="space-y-4">
          <Card className="p-5">
            ...
            <div className="space-y-2">
              {bedTotals.slice(0,5).map(({b, kg}, i) => (
                <div key={b.id} className="flex items-center gap-3 p-2 rounded hover:bg-accent">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[10px] text-muted-foreground/60">{b.lengthM > 0 ? (kg/b.lengthM).toFixed(2) : "0.00"} kg/m</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Card className="p-5">
            ...
            <div className="text-3xl font-bold text-primary">{(totalKg * 0.5 + 25).toFixed(0)} <span className="text-base font-normal text-muted-foreground">kg projected</span></div>
            <div className="text-xs text-muted-foreground mt-1">Based on current bed maturity & 7-day trend</div>
          </Card>
        </TabsContent>
              ...
              {Object.entries(byVariety).sort((a,b)=>b[1]-a[1]).map(([variety, kg]) => {
                ...
                return (
                  <div key={variety} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                      <div className="h-full bg-primary rounded-full" style={{ width: `${(kg/max)*100}%` }} />
                    </div>
        ...
        <TabsContent value="packaging" className="space-y-4">
          ...
          <Card className="p-5">
            ...
            <p className="text-xs text-muted-foreground mb-4">Total cartons and plates packed per irrigation zone</p>
                ...
                return (
                  <div key={v.id}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-muted-foreground">📦 {cartons} cartons · 🍽️ {plates} plates</span>
                    </div>
          ...
          <Card className="p-5">
            ...
            <p className="text-xs text-muted-foreground mb-4">Which strawberry variety produces the most cartons</p>
            <div className="space-y-3">
              {Object.entries(cartonsPerVariety).sort((a, b) => b[1] - a[1]).map(([variety, cartons]) => {
                const plates = platesPerVariety[variety] ?? 0;
                return (
                  <div key={variety} className="flex items-center gap-3">
                    <div className="w-44 text-xs font-medium text-foreground truncate shrink-0">{variety}</div>
                    <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                    ...
                    <div className="text-xs tabular-nums w-28 text-right shrink-0">
                      <span className="font-bold text-foreground">{cartons}</span>
          ...
          <Card className="p-5">
            ...
            <p className="text-xs text-muted-foreground mb-4">Breakdown of packaging destination — export, juice, jam, local, hotel, supermarket</p>
            <div className="space-y-3">
              {Object.entries(cartonsByPurpose).sort((a, b) => b[1] - a[1]).map(([purpose, cartons]) => {
                ...
                return (
                  <div key={purpose} className="flex items-center gap-3">
                    <div className="w-28 text-xs font-medium text-foreground capitalize shrink-0">{purpose}</div>
                    <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
