...
export default function FollowUpsPage() {
  ...
  function deleteFollowUp(id: string) {
    const idx = FOLLOW_UPS.findIndex(f => f.id === id);
    if (idx >= 0) FOLLOW_UPS.splice(idx, 1);
    setFollowUps([...FOLLOW_UPS]);
                  ...
                  <div className="flex items-start gap-2 flex-wrap">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                        <Badge className={`text-[10px] capitalize ${PRIORITY_STYLE[fu.priority]}`}>
                          {fu.priority}
                        </Badge>
                        <span className="text-[10px] text-slate-400 capitalize">
                          {FOLLOW_UP_ENTITY_LABELS[fu.entityType]}
                        </span>
                        ...
                        {worker && (
                          <span className="text-[10px] text-slate-500">→ {worker.name.split(" ")[0]}</span>
                        )}
                      </div>
                      {/* Completion note */}
                      {fu.completionNote && (
