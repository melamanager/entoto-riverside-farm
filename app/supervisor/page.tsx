  ...
  return (
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-foreground">{t.supervisor.title}</h1>
          </div>
      ...
      {supervisors.map(sup => {
        ...
        return (
          <Card key={sup.id} className="border border-border shadow-sm overflow-hidden">
            ...
            <div className="flex items-center gap-4 px-6 py-4 bg-muted border-b border-border">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-base font-bold text-foreground">{sup.name}</span>
                  <Badge className="bg-primary/15 text-primary border-primary/30 text-[10px]">Supervisor</Badge>
                </div>
              ...
              <div className="hidden md:flex items-center gap-6 text-center">
                <div>
                  <div className="text-xl font-bold text-foreground">{myBeds.length}</div>
                  <div className="text-[10px] text-muted-foreground">{t.supervisor.beds}</div>
                </div>
                <div>
                  <div className="text-xl font-bold text-primary">{todayHarvestKg.toFixed(1)}</div>
                  <div className="text-[10px] text-muted-foreground">{t.supervisor.kgToday}</div>
                </div>
                <div>
                  <div className="text-xl font-bold text-red-600">{myDiseases.length}</div>
                  <div className="text-[10px] text-muted-foreground">{t.supervisor.alerts}</div>
                </div>
                <div>
                  <div className="text-xl font-bold text-amber-600">{presentCount}/{todayAttSup.length}</div>
                  <div className="text-[10px] text-muted-foreground">{t.supervisor.present}</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 divide-y lg:divide-y-0 lg:divide-x divide-border">
              ...
              <div className="p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-foreground/80 flex items-center gap-1.5"><Sprout className="size-3.5 text-primary"/>Bed Status</h3>
                  <Link href="/beds" className="text-[11px] text-primary hover:underline">View all →</Link>
                </div>
                  ...
                  {myBeds.slice(0, 6).map(bed => {
                    ...
                    return (
                      <Link href={`/beds/${bed.id}`} key={bed.id} className="flex items-center gap-3 group">
                        ...
                        <span className="font-mono text-xs font-semibold text-foreground/80 group-hover:text-primary w-20">{bed.id}</span>
                        <span className="text-[11px] text-muted-foreground truncate flex-1">{bed.variety.split(" ")[0]}</span>
                        <span className="text-[11px] font-semibold text-foreground/70 tabular-nums">{kg.toFixed(1)}kg</span>
                      </Link>
                    );
                  })}
                  {myBeds.length > 6 && <div className="text-[11px] text-muted-foreground">+{myBeds.length-6} more beds</div>}
                </div>
              </div>
              ...
              <div className="p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-foreground/80 flex items-center gap-1.5"><Bug className="size-3.5 text-red-500"/>Disease Alerts</h3>
                  <Link href="/diseases" className="text-[11px] text-primary hover:underline">Manage →</Link>
                </div>
                {myDiseases.length === 0 ? (
                    ...
                    {myDiseases.slice(0, 4).map(d => (
                      <div key={d.id} className="p-2.5 rounded-lg border border-red-100 bg-red-50/50">
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-mono text-[11px] font-bold text-foreground/80">{d.bedId}</span>
                          <Badge variant="destructive" className="text-[9px] h-4">{d.severity}%</Badge>
                        </div>
                        <div className="text-[11px] font-semibold text-foreground/80">{DISEASE_LABELS[d.type]}</div>
              ...
              <div className="p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-foreground/80 flex items-center gap-1.5"><ListChecks className="size-3.5 text-blue-500"/>My Tasks</h3>
                  <Link href="/tasks" className="text-[11px] text-primary hover:underline">All tasks →</Link>
                </div>
                <div className="flex gap-3 mb-3 text-center">
                  ...
                  <div className="flex-1 rounded-lg bg-primary/10 border border-primary/20 p-2">
                    <div className="text-lg font-bold text-primary">{doneTasks.length}</div>
                ...
                <div className="space-y-2">
                  {pendingTasks.slice(0,3).map(task => (
                    <div key={task.id} className="flex items-center gap-2.5">
                      <span className={`size-2 rounded-full shrink-0 ${task.priority==="high"?"bg-red-500":task.priority==="medium"?"bg-amber-500":"bg-muted-foreground"}`} />
                      <span className="text-xs text-foreground/80 truncate">{task.title}</span>
                    </div>
                  ))}
                  {pendingTasks.length === 0 && <div className="text-xs text-primary flex items-center gap-1"><CheckCircle2 className="size-3"/>All tasks complete</div>}
                </div>
              </div>
                ...
                return (
                  <div className="p-5">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Link href="/follow-ups" className="text-[11px] text-primary hover:underline">View all →</Link>
                    </div>
                    {myFollowUps.length === 0 ? (
                      <div className="flex items-center gap-1.5 text-xs text-primary"><CheckCircle2 className="size-3"/>All clear</div>
                    ) : (
                      <div className="space-y-2">
                        {shownFUs.map(fu => {
                        ...
                        {myFollowUps.length > 4 && (
                          <div className="text-[11px] text-muted-foreground">+{myFollowUps.length - 4} more follow-ups</div>
                        )}
                      </div>
                    )}
                  </div>
      ...
      <Card className="border border-border shadow-sm p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-foreground flex items-center gap-2"><CalendarCheck className="size-4 text-primary" />{t.supervisor.attendanceSummary}</h3>
          <Link href="/attendance" className="inline-flex items-center gap-1 text-xs text-primary hover:underline font-medium">
