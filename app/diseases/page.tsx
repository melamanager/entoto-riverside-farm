...
export default function DiseasesPage() {
  ...
  function markResolved(d: DiseaseReport) {
    ...
    if (d.requiresImageProof && !d.proofImageUrl) {
      toast.error("Cannot resolve — supervisor has not uploaded proof photo yet."); return;
    }
    confirmResolve(d.id);
  }
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto space-y-6">
                    ...
                    <div className="flex items-start gap-4 flex-wrap">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <span className={`text-xs font-bold tabular-nums ${d.severity > 60 ? "text-red-600" : d.severity > 30 ? "text-amber-600" : "text-emerald-600"}`}>{d.severity}%</span>
                        </div>
                        {d.infectedLengthM !== undefined && d.infectedLengthM > 0 && (() => {
                          const b = getBed(d.bedId);
                          const pct = b ? Math.round((d.infectedLengthM / b.lengthM) * 100) : null;
                          return (
                            <div className="flex items-center gap-2 mb-3">
                              <span className="text-[11px] text-muted-foreground w-16">Infected</span>
                              <div className="h-2 bg-muted rounded-full flex-1 overflow-hidden">
                                <div className={`h-full rounded-full ${d.severity > 60 ? "bg-red-400" : d.severity > 30 ? "bg-amber-400" : "bg-emerald-400"}`}
                                  style={{ width: `${pct ?? 0}%` }} />
                              </div>
                              <span className="text-xs font-bold tabular-nums text-foreground/80">{d.infectedLengthM}m{pct !== null ? ` (${pct}%)` : ""}</span>
                            </div>
                          );
                        })()}
                        {d.managerRecommendation && (
                          <div className="mb-2 bg-amber-50 border border-amber-200 rounded-lg p-3">
                            <div className="flex items-center gap-1.5 mb-1">
                              ...
                              {d.notifiedAt && <span className="text-[10px] text-amber-600 ml-auto">{new Date(d.notifiedAt).toLocaleTimeString("en", { hour: "2-digit", minute: "2-digit" })}</span>}
                            </div>
                        ...
                        {d.treatmentApplied && (
                          <div className="mt-2 space-y-1.5">
                            ...
                            {d.treatmentNote && (
                              <div className="text-[11px] text-foreground/70 italic bg-muted border border-border rounded px-2.5 py-1.5">"{d.treatmentNote}"</div>
                            )}
                            {d.proofImageUrl && (
                      ...
                      <div className="flex flex-col gap-2 shrink-0 min-w-[200px]">
                        {isManager && group.key === "open" && (
                          <Button size="sm" className="gap-1.5 bg-blue-600 hover:bg-blue-700 text-xs w-full" onClick={() => openRecommend(d)}>
                            <MessageSquare className="size-3.5" /> {t.diseases.writeRecommendation}
                          </Button>
                        )}
                        {isManager && group.key === "treating" && (
                          <Button size="sm" className="gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-xs w-full" onClick={() => markResolved(d)}>
                        ...
                        {isSupervisor && group.key === "notified" && (
                          <Button size="sm" className="gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-xs w-full" onClick={() => openConfirm(d)}>
                            <CheckCircle2 className="size-3.5" /> {t.diseases.confirmTreatment}
                          </Button>
                        )}
                        <Button size="sm" variant="ghost" className="gap-1.5 text-xs w-full text-muted-foreground" onClick={() => setExpandedId(isExpanded ? null : d.id)}>
                    ...
                    {isExpanded && (
                      <div className="mt-4 border-t border-border pt-4">
                        ...
                        <div className="space-y-2">
                          {steps.map((step, i) => (
                            <button key={i} onClick={() => toggleStep(d.id, i, steps.length)} className="flex items-start gap-3 w-full text-left group">
      ...
      {STATUS_GROUPS.every(g => g.items.length === 0) && (
      ...
      <Dialog open={recommendOpen} onOpenChange={o => !o && setRecommendOpen(false)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><MessageSquare className="size-4 text-blue-600" /> Write Treatment Recommendation</DialogTitle>
          </DialogHeader>
          {recommendTarget && (
            <div className="space-y-4">
              <div className="bg-muted border border-border rounded-lg p-3 text-sm">
                ...
                <div className="text-muted-foreground text-xs mt-0.5">Bed {recommendTarget.bedId} · Severity {recommendTarget.severity}% · Reported {new Date(recommendTarget.reportedAt).toLocaleDateString("en", { month: "short", day: "numeric" })}</div>
              </div>
              <div>
                <label className="text-xs font-semibold text-foreground/80 block mb-1.5">Treatment Recommendation <span className="text-red-500">*</span></label>
                <Textarea value={recommendation} onChange={e => setRecommendation(e.target.value)}
                  placeholder="Describe exact treatment: chemicals, doses, application method, timing…" rows={5} className="text-sm resize-none" />
              ...
              <button type="button" onClick={() => setRequireImage(p => !p)}
              ...
              <Button className="w-full bg-blue-600 hover:bg-blue-700 gap-2" disabled={!recommendation.trim()} onClick={sendRecommendation}>
      ...
      <Dialog open={confirmOpen} onOpenChange={o => !o && setConfirmOpen(false)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><CheckCircle2 className="size-4 text-emerald-600" /> Confirm Treatment Applied</DialogTitle>
          </DialogHeader>
          {confirmTarget && (
            <div className="space-y-4">
              <div className="bg-muted border border-border rounded-lg p-3 text-sm">
                <div className="font-semibold text-foreground">{DISEASE_LABELS[confirmTarget.type]}</div>
                <div className="text-muted-foreground text-xs mt-0.5">Bed {confirmTarget.bedId} · Severity {confirmTarget.severity}%</div>
              </div>
              {confirmTarget.managerRecommendation && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                  <div className="flex items-center gap-1.5 mb-1"><ShieldCheck className="size-3.5 text-amber-600" /><span className="text-xs font-semibold text-amber-800">Manager's Instructions</span></div>
              ...
              <div>
                ...
                <div className="space-y-2 max-h-[180px] overflow-y-auto pr-1">
                  {(confirmTarget.treatmentSteps ?? DISEASE_TREATMENT_STEPS[confirmTarget.type]).map((step, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => toggleStep(confirmTarget.id, i, (confirmTarget.treatmentSteps ?? DISEASE_TREATMENT_STEPS[confirmTarget.type]).length)} className="flex items-start gap-3 w-full text-left group">
              ...
              <div>
                <label className="text-xs font-semibold text-foreground/80 block mb-1.5">Treatment Note <span className="text-red-500">*</span></label>
                <Textarea value={treatmentNote} onChange={e => setTreatmentNote(e.target.value)}
                  placeholder="Describe what was applied, quantities used, and any observations…" rows={3} className="text-sm resize-none" />
              </div>
              <div>
                ...
                {proofImage ? (
                  <div className="relative rounded-lg overflow-hidden border border-border">
                    ...
                    <div className="absolute top-2 right-2 flex gap-1">
                      <button onClick={() => setPreviewUrl(proofImage)} className="size-7 rounded-full bg-black/60 text-white grid place-items-center hover:bg-black/80"><Eye className="size-3.5" /></button>
                      <button onClick={() => { setProofImage(null); setProofImageName(""); }} className="size-7 rounded-full bg-black/60 text-white grid place-items-center hover:bg-black/80"><X className="size-3.5" /></button>
                    </div>
                  ...
                  <button type="button" onClick={() => proofInputRef.current?.click()}
                    ...
                    <div className="text-xs text-muted-foreground"><span className={`font-semibold ${confirmTarget.requiresImageProof ? "text-purple-600" : "text-foreground/70"}`}>Click to upload</span> or take photo</div>
              ...
              <Button className="w-full bg-emerald-600 hover:bg-emerald-700 gap-2"
                disabled={!treatmentNote || (confirmTarget.requiresImageProof && !proofImage)} onClick={submitTreatment}>
      ...
      <Dialog open={!!proofReviewTarget} onOpenChange={o => !o && setProofReviewTarget(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Eye className="size-4 text-purple-600" /> Review Supervisor Proof Before Resolving</DialogTitle>
          </DialogHeader>
          {proofReviewTarget && (
            <div className="space-y-4">
              <div className="bg-muted border border-border rounded-lg p-3 text-sm">
              ...
              {proofReviewTarget.treatmentNote && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800 italic">"{proofReviewTarget.treatmentNote}"</div>
              )}
              <div>
              ...
              <div className="flex gap-2">
                ...
                <Button className="flex-1 bg-emerald-600 hover:bg-emerald-700 gap-2" onClick={() => confirmResolve(proofReviewTarget.id)}>
      ...
      {previewUrl && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={() => setPreviewUrl(null)}>
          <div className="relative max-w-2xl w-full" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setPreviewUrl(null)} className="absolute top-3 right-3 size-8 rounded-full bg-black/60 text-white grid place-items-center hover:bg-black/80"><X className="size-4" /></button>
          </div>
        </div>
      )}
    </div>
