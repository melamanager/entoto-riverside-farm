  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4"><div className="flex items-center gap-2 text-xs text-muted-foreground"><Sprout className="size-3.5" /> Beds</div><div className="text-3xl font-bold mt-1">{beds.length}</div></Card>
        <Card className="p-4"><div className="flex items-center gap-2 text-xs text-muted-foreground"><ValveIcon size={14} /> Plants</div><div className="text-3xl font-bold mt-1">{beds.reduce((s, b) => s + plantsInBed(b), 0).toLocaleString()}</div></Card>
        <Card className="p-4"><div className="flex items-center gap-2 text-xs text-muted-foreground"><Wheat className="size-3.5" /> Total harvest</div><div className="text-3xl font-bold mt-1">{totalKg.toFixed(1)}<span className="text-sm font-normal text-muted-foreground"> kg</span></div></Card>
        <Card className="p-4"><div className="flex items-center gap-2 text-xs text-muted-foreground"><AlertTriangle className="size-3.5" /> Active alerts</div><div className="text-3xl font-bold mt-1 text-rose-600">{diseases.filter(d => d.status !== "resolved").length}</div></Card>
      </div>

      {cameraAlerts.length > 0 && (
        <Card className="p-5">
          <h3 className="font-bold mb-3 flex items-center gap-2"><Camera className="size-4 text-muted-foreground" /> Camera Alerts — {valve.name}</h3>
              ...
              <tbody>
                {soilReadings.map(sr => (
                  <tr key={sr.bedId} className="border-b border-border/50 hover:bg-accent/30">
                    <td className="py-1.5 pr-4 font-mono font-semibold"><Link href={`/beds/${sr.bedId}`} className="hover:text-primary">{sr.bedId}</Link></td>
                    <td className="py-1.5 pr-4 text-right tabular-nums"><span className={sr.moisturePct < 50 ? "text-amber-600 font-bold" : ""}>{sr.moisturePct}%</span></td>
      ...
      <Card className="p-5">
        <h3 className="font-bold mb-4 flex items-center gap-2 text-foreground"><Calendar className="size-4 text-muted-foreground" /> Zone Activity Log</h3>
