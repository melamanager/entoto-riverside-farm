...
const RISK_STYLE = {
  ...
  Low:    { badge: "bg-primary/15 text-primary border border-primary/30", dot: "bg-primary", bar: "bg-primary" },
} as const;
...
export function WeatherWidget() {
  ...
  if (!weather) {
    return (
      <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden h-full flex flex-col items-center justify-center text-muted-foreground text-sm">
        Loading weather…
      </div>
    );
  }
    ...
    <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden h-full flex flex-col">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-1.5 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-2">
              {weather.source === "live" ? (
                <><Wifi className="size-3 text-primary" /> Live · Addis Ababa</>
              ) : (
                <><WifiOff className="size-3 text-amber-500" /> Simulated · Add Tomorrow.io key</>
              )}
            </div>
            <div className="flex items-end gap-3">
              <span className="text-5xl font-black text-foreground tracking-tight">{weather.tempC}°</span>
              <span className="text-4xl leading-none mb-1">{weather.conditionIcon}</span>
            </div>
            <div className="text-sm font-medium text-muted-foreground mt-1">{weather.condition}</div>
          </div>
          ...
          <div className="space-y-2.5 text-right shrink-0">
            ...
            <div className="flex items-center gap-1.5 justify-end">
              <div className="text-right">
                <div className="text-sm font-bold text-foreground">{weather.windKph} km/h</div>
            ...
            <div className="flex items-center gap-1.5 justify-end">
              <div className="text-right">
                <div className="text-sm font-bold text-foreground">UV {weather.uvIndex}</div>
                <div className="text-[10px] text-muted-foreground">{UV_LABEL(weather.uvIndex)}</div>
              </div>
      ...
      <div className="px-5 py-4 flex-1">
        <div className="flex items-center justify-between mb-3">
          <div className="text-xs font-bold text-foreground uppercase tracking-wider">Disease Risk Today</div>
          {highRisks > 0 ? (
            ...
            <div className="flex items-center gap-1 text-[11px] text-primary font-bold bg-primary/10 border border-primary/30 px-2 py-0.5 rounded-full">
            ...
            return (
              <div key={r.name} className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-muted-foreground leading-snug">{r.reason}</div>
                </div>
              </div>
            );
          })}
